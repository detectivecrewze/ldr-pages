...
const renderers = {
    ...
    renderPageStep(step) {
        // 1. Check for Modular Editors (Studio-Style)
        if (step.id === 'general' && window.ThemeEditor) return ThemeEditor.render();
        if (step.type === 'dashboard' && window.DashboardEditor) return DashboardEditor.render();
        if (step.type === 'journey' && window.JourneyEditor) return JourneyEditor.render();
        if (step.type === 'shared' && window.SharedEditor) return SharedEditor.render();
        if (step.type === 'messenger' && window.MessengerEditor) return MessengerEditor.render();
        if (step.type === 'bucketlist' && window.BucketEditor) return BucketEditor.render();
        if (step.type === 'quiz' && window.QuizEditor) return QuizEditor.render();
        if (step.type === 'letter' && window.LetterEditor) return LetterEditor.render();
        if (step.type === 'video' && window.VideoEditor) return VideoEditor.render();
        if (step.type === 'sweeper' && window.SweeperEditor) return SweeperEditor.render();

        // 2. Legacy: Find the original step definition in WIZARD_STEPS
