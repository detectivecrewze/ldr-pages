...
const app = {
    ...
    init() {
        ...
        // Check if first time
        if (!localStorage.getItem('ldr_studio_onboarded')) {
    ...
    closeWelcomeWizard() {
        ...
        localStorage.setItem('ldr_studio_onboarded', 'true');
    },
    ...
    renderStep() {
        ...
        // Check for Studio-Style Modular Renderers
        const stepPage = state.getPages(false).find(p => p.id === step.id || p.type === step.type);

        if (content) {
            if (stepPage && renderers.renderPageStep) {
                // Delegate to renderers.js which now knows about ThemeEditor, DashboardEditor, etc.
                content.innerHTML = renderers.renderPageStep(stepPage);
            } else if (step && step.render) {
    ...
    onPreviewLoad() {
        console.log('[Studio] Preview iframe loaded');
